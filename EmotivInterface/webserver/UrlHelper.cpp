...
std::string EncodeUTF8(std::string querystr)
{	
	std::string::size_type next_amp = 0;	    
	next_amp = querystr.find("\\u");

	while( next_amp != std::string::npos )
	{
		std::string ch_value = querystr.substr(next_amp,6);
		char   *string, *stopstring; 
		string = &ch_value[2];
		const char ch[2] = {(char)strtol(string, &stopstring, 16), '\0'};
		querystr = ReplaceInStr(querystr, ch_value, ch);	
		next_amp = querystr.find("\\u");
	}
...
void ParseParams(std::string querystr, std::map<std::string, std::string>& params)
{
std::string::size_type next_amp = 0;
	
    querystr += "&";
	next_amp = querystr.find("&");
	while (next_amp != std::string::npos) 
	{
		std::string name_value = querystr.substr(0,next_amp);
		querystr               = querystr.substr(next_amp+1);
		next_amp               = querystr.find("&");
